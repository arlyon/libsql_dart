        ...
        move || {
            ...
            move |context| async move {
                transform_result_sse::<_, String>(
                    (move || async move {
                        let output_ok = crate::api::libsql::connect(api_args).await?;
                        Ok(output_ok)
                    })()
                    .await,
                )
...
fn pde_ffi_dispatcher_primary_impl(
    ...
    match func_id {
        ...
        6 => wire__crate__api__connection__libsql_connection_prepare_impl(
            port,
            ptr,
            rust_vec_len,
            data_len,
        ),
        7 => wire__crate__api__connection__libsql_connection_query_impl(
            port,
            ptr,
            rust_vec_len,
            data_len,
        ),
        8 => wire__crate__api__connection__libsql_connection_sync_impl(
            port,
            ptr,
            rust_vec_len,
            data_len,
        ),
        9 => wire__crate__api__connection__libsql_connection_transaction_impl(
            port,
            ptr,
            rust_vec_len,
            data_len,
